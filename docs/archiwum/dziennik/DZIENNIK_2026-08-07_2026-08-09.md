# ARCHIWUM DZIENNIKA — RelAI · 2026-08-07 … 2026-08-09

Zarchiwizowano: 2026-08-17 · Źródło: `docs/DZIENNIK.md` · Wpisów: 16 · Suma kontrolna
przeniesionej treści: `c17de1981ceedb1c` (SHA-256, pierwsze 16 znaków, końce linii LF)
...
**Do zrobienia przez człowieka:**
- Zgodność `docs/DECYZJE.md` tego repo ze świeżą `SPEC_DECYZJE.md`: **rejestr jest zgodny co do zasady i grup tematycznych**, różni się trzema rzeczami — (1) wpisy nie mają dat indywidualnych, tylko zbiorczą informację w nagłówku („wywiad 2026-08-07"), (2) część wpisów nie ma jawnego powodu, (3) zmiana D-38 jest odnotowana w treści wpisu zamiast w sekcji „Decyzje zmienione". Decyzja, czy migrować rejestr do nowego formatu (~90 wpisów), czy zostawić jako historyczny — należy do Łukasza. *(rozstrzygnięte 2026-08-17 — rejestr zostaje historyczny; format ze specyfikacji obowiązuje wyłącznie nowe wpisy, pierwszy taki to D-86)*
...
**Do zrobienia przez człowieka:**
- Uruchomić E4: świeża sesja Claude Code w folderze `RelAI`, model **Opus**, polecenie: „Wykonaj *(zrobione 2026-08-07 — E4 zrealizowany, wersja 0.4.0)*
  docs/plany/BUDOWA_RELAI/PROMPT_ETAP_4.md".
- Rozstrzygnąć, czy sekcja „Weryfikacja" ma wejść do wzorca etapu w `SPEC_PLAN.md` (dogfooding *(rozstrzygnięte 2026-08-17 — zostaje wyłącznie w prompcie etapowym; plan opisuje cel i zakres, sprawdzalne punkty żyją tam, gdzie są wykonywane)*
...
**Do zrobienia przez człowieka:**
- Decyzja, czy plugin ma zostać zainstalowany na stałe w trakcie dalszej budowy (teraz jest, scope *(rozstrzygnięte 2026-08-07 — zainstalowany na stałe, wiersz w `docs/USTAWIENIA.md`)*
...
**Do zrobienia przez człowieka:**
- **Potwierdzić rezygnację z osobnego skilla `relai-stages`** (sekcja 5.1 planu) — albo polecić *(rozstrzygnięte 2026-08-07 — skille to `relai-core` i `relai-planning`; etap uruchamia komenda `/relai-stage`)*
  jego wydzielenie jako aneks do planu.
- **Rozstrzygnąć kierunek dla R8** przed E5: czy specyfikacje zostają plikami w `templates/` *(rozstrzygnięte 2026-08-07 — R8 zamknięte w E5: hook kopiuje specyfikacje do `.claude/relai/templates/` w projekcie)*
...
- Uruchomić E5: świeża sesja Claude Code w folderze `RelAI`, model **Opus**, polecenie: „Wykonaj *(zrobione 2026-08-07 — E5 zrealizowany, wersja 0.5.0)*
...
**Do zrobienia przez człowieka:**
- **Potwierdzić doprecyzowanie konwencji hook-guard** (README, sekcja hook-guard): dla zdarzenia *(rozstrzygnięte 2026-08-17 — konwencja obowiązuje w wydanych hookach od 0.5.0 i przeszła bez zastrzeżeń przez pięć wersji oraz pilotaż E6; potwierdzenie milczące)*
...
- Zdecydować, czy `config-protection` ma pytać („ask") także przy dopisywaniu wierszy preferencji *(rozstrzygnięte 2026-08-17 — nie pyta przy dopisywaniu wiersza preferencji; bramka zostaje na zmianie i usunięciu istniejących wierszy)*
...
- Uruchomić E6 wg `PROMPT_ETAP_6.md`: generacja pięciu propozycji — świeża sesja **Opus**; *(zrobione 2026-08-07 — propozycje wygenerowane, konkurs rozstrzygnięty 2026-08-08)*
...
**Do zrobienia przez człowieka:**
- **Wybrać 1–2 propozycje** w sesji **Fable** (D-85): otworzyć pięć plików z *(rozstrzygnięte 2026-08-08 — kierunki odrzucone na stałe w D-61b, zachowana „Redakcja"; finał to blend „Warsztat")*
  `docs/zasoby/design-konkurs/` i powiedzieć, które kierunki wchodzą do iteracji oraz co w nich
  zmienić. Bez tego etap E6 stoi.
- Rozstrzygnąć, czy wybrany kierunek ma trafić także do **warstwy globalnej** *(rozstrzygnięte 2026-08-17 — kierunek zostaje wpisem projektowym; warstwa globalna wpisu o designie nie ma i nie dostaje)*
...
**Do zrobienia przez człowieka:**
- **Obejrzeć pięć propozycji z `runda-2/` i wskazać 1–2 kierunki** do iteracji wraz z uwagami. *(zrobione 2026-08-08 — runda 2 obejrzana, kierunek wybrany w rundzie 3)*
  Bez tego etap E6 stoi.
- Rozstrzygnąć, czy waga pliku rzędu 200–280 KB jest akceptowalna dla planu wysyłanego klientowi, *(rozstrzygnięte 2026-08-17 — komplet znaków zostaje, bez podzbioru; identyczny wygląd na każdej maszynie jest wart tej wagi)*
...
**Do zrobienia przez człowieka:**
- Obejrzeć blend i powiedzieć, czy to jest już ten kierunek, czy potrzeba kolejnej iteracji. *(rozstrzygnięte 2026-08-08 — blend „Warsztat" zaakceptowany, wiersz w `docs/USTAWIENIA.md`)*
...
**Do zrobienia przez człowieka:**
- Zdecydować, czy resztę E6 (nadpisania lokalne D-62, honorowanie preferencji formatu, wersja *(rozstrzygnięte 2026-08-08 — reszta E6 dokończona w tej samej wersji 0.6.0)*
...
**Do zrobienia przez człowieka:**
- Przy pierwszym własnym planie HTML: sprawdzić, czy pytanie o zmianę stylu pada raz i czy plan *(rozstrzygnięte 2026-08-17 — zmierzone w pilotażu E6: pytanie o szablon padło dokładnie raz, po pierwszym planie)*
  otwiera się bez przewijania w poziomie.
- Decyzja o podzbiorze fontów (waga pliku kontra komplet znaków) — patrz R5. *(rozstrzygnięte 2026-08-17 — komplet znaków, bez podzbioru)*
...
**Do zrobienia przez człowieka:**
- Potwierdzić w sesji interaktywnej, czy podpowiadacz rozwija skróconą formę `/relai-backup`. *(rozstrzygnięte 2026-08-08 — L-0022: forma skrócona nie istnieje, komendy żyją w przestrzeni `/relai:relai-*`)*
  Jeśli nie — zdanie o przedrostku w `KOMENDY.md` przestaje być przypisem i staje się główną
  formą w kolumnie „Komenda".
- Wskazać docelową lokalizację centralnego folderu backupów dla tej maszyny (wpis trafi do *(rozstrzygnięte 2026-08-10 — `C:UsersLukaszBackupyRelAI`, wiersz w ustawieniach globalnych)*
  `~/.claude/relai/USTAWIENIA.md`).
- Decyzja o podzbiorze fontów wraca — pakiet przekazania waży 201 KB z tego samego powodu co plan *(rozstrzygnięte 2026-08-17 — komplet znaków, bez podzbioru)*
  (R5).
...
**Do zrobienia przez człowieka:**
- Wskazać profil tego repo jedną z czterech wartości (`app` / `agent-voice` / `flow` / `prompty`), *(rozstrzygnięte 2026-08-07 — profil zapisany w `docs/USTAWIENIA.md`: narzędzie/plugin, odpowiednik profilu „prompty")*
...
- Zdecydować, czy `docs/ARCHITEKTURA.md` ma powstać dla tego repo — reguła profilu `app` zażąda go *(rozstrzygnięte 2026-08-17 — nie powstaje; jego rolę pełnią `core/README.md` i `docs/PRZENOSNOSC.md`)*
  przy pierwszym pliku źródłowym po zmianie wiersza wyżej.
- Potwierdzić w sesji interaktywnej, że pytanie o testy i pytanie o kierunek wizualny padają jako *(rozstrzygnięte 2026-08-17 — pilotaż E6: oba pytania padły w Cursorze przy pierwszym kodzie i pierwszym UI, jako pojedyncze pytania)*
...
**Do zrobienia przez człowieka:**
- Uruchomić E10: świeża sesja **Opus**, polecenie „Wykonaj docs/plany/BUDOWA_RELAI/PROMPT_ETAP_10.md". *(zrobione 2026-08-10 — E10 zrealizowany, wydanie 1.0.0)*
