...
| `/relai-prompt <tekst>` | bierze zdanie podyktowane w biegu i oddaje z niego prompt gotowy do wykonania: dokłada format wyjścia, kryterium odbioru i granicę zakresu, oznacza każde swoje dopowiedzenie, usuwa wartości kluczy i haseł. Pokazuje Twój oryginał obok propozycji i zatrzymuje się — niczego nie wykonuje i nie zapisuje | gdy dyktujesz polecenie szybko i wolisz zobaczyć poprawioną wersję, zanim agent zacznie pracować |
