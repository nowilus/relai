...
- **D-88** *(2026-09-24, przy akceptacji planu PROWADZENIE_END_TO_END)* **Rotacja dokumentu zabiera najstarsze pozycje, aż waga całego żywego pliku zejdzie poniżej 60% jego progu** — nie „część rotowalna poniżej 60%". Powód (zmierzony dwa razy, 2026-09-04 i 2026-09-14): cel liczony od części rotowalnej bywa nieosiągalny z definicji w dokumencie o grubej dolnej granicy — litera specyfikacji zostawiłaby dziennik na 164,8 KB przy progu 150 i nie wzięłaby z lekcji ani jednej pozycji, a kryterium całego pliku dało 87,3 KB i 42,6 KB. Zmiana działa we wszystkich projektach; wdrożenie w `SPEC_ARCHIWUM.md` i w mechanizmie — etap E2 planu.
...
- **D-89** *(2026-09-04, po wątku CURSOR_1_9_1; do 2026-09-24 zapisana omyłkowo jako drugie D-87 — przenumerowana w E1 planu PROWADZENIE_END_TO_END, treść bez zmian)* **Repozytorium RelAI nie dostaje adaptera Cursora.** Ten projekt prowadzi się w Claude Code i tak zostaje; `node adapters/cursor/install.js .` nie jest tu uruchamiane. Powód jest architektoniczny, nie wygodowy: instalacja kładzie w `.cursor/` **kopie dwunastu komend i dwóch skilli**, których oryginały leżą w tym samym repozytorium — a `.cursor/` nie jest ignorowany przez gita, więc źródło i kopia trafiłyby do jednego drzewa i rozjechały się przy pierwszej zmianie komendy. Konsekwencja przyjęta świadomie: sesja otwarta w Cursorze **w tym folderze** nie dostaje kontekstu RelAI ani hooka blokującego sekret, a `/relai-models` kończy się tam na Kroku 1. Adapter Cursora mierzy się na **projektach kontrolnych** (`%TEMP%`), nie na własnym repozytorium — tak zrobił wątek CURSOR_1_9_1 i to jest wzorzec na przyszłość. Zmiana tej decyzji wymaga wcześniejszego wpisu `.cursor/` do `.gitignore`.
...
- **D-85** *(wyjątek E7, 2026-09-05)* Etap E7 planu ROZWOJ_PO_WYDANIU wykonuje **`gpt-5.6-terra` z effortem `high` w Codeksie**. To świadomy wyjątek kosztowy zatwierdzony przed rozpoczęciem implementacji; wcześniejsza propozycja `gpt-5.6-sol/high` nie weszła do realizacji. Pozostałe etapy zachowują dotychczasową decyzję.
...
| **D-40** → aneks | 2026-09-24 | Hooków jest **11**, nie 8: do ośmiu wymienionych doszły `journal-signature`, `profile-rules` i `prompt-mode`. Zasady D-40 bez zmian — jeden plik Node.js na hook, bez par `.ps1`/`.sh`; podział na blokujące i ostrzegające zostaje w D-41. | Treść D-40 wyliczała stan z v1 i rozjechała się z `adapters/claude-code/hooks/` (pozycja A04 planu PROWADZENIE_END_TO_END, etap E1); decyzji zamrożonej się nie przepisuje, więc stan dopisuje aneks. |
| **D-80** → aneks | 2026-09-24 | „Wsparcie Cursor/Codex" przestało być poza zakresem: adaptery Cursora i Codeksa są dostarczone od wydań 2.x (trzy adaptery w 2.3.x). Pozostałe wykluczenia D-80 — telemetria kosztów, pełne tłumaczenie EN, infrastruktura feedbacku, GUI — obowiązują bez zmian. | D-80 opisywała granicę v1, a plan ROZWOJ_PO_WYDANIU świadomie ją przekroczył; bez aneksu decyzja zaprzeczała stanowi repozytorium (pozycja A04, etap E1). |
