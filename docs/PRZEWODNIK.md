...
Wszystko, czego nie trzeba wiedzieć w pierwszych trzydziestu minutach — przeniesione z `README.md`
w wydaniu 2.7.0. Instalacja: [INSTALACJA.md](INSTALACJA.md); pierwsze kroki i słowniczek:
[README](../README.md).
...
<p align="center">
  <img src="zasoby/branding/diagram.svg" alt="Jak działa RelAI: rozmawiasz, RelAI zapisuje, pliki zostają, nowa sesja czyta" width="900">
</p>
...
| Komenda | Co robi |
|---|---|
| <img src="zasoby/branding/ikony/stage.svg" width="24" align="absmiddle"> `/relai-stage` | uruchamia kolejny etap aktywnego planu — pokazuje, co zrobi, i **czeka** na zgodę |
| <img src="zasoby/branding/ikony/backup.svg" width="24" align="absmiddle"> `/relai-backup` | pakuje projekt do ZIP-a w centralnym folderze kopii, z twardym wykluczeniem sekretów |
| <img src="zasoby/branding/ikony/audit.svg" width="24" align="absmiddle"> `/relai-audit` | raport o stanie dokumentacji zakończony listą propozycji — sam niczego nie zmienia |
| <img src="zasoby/branding/ikony/changelog.svg" width="24" align="absmiddle"> `/relai-changelog` | destyluje dziennik do listy zmian; na ekran, do pliku dopiero na życzenie |
| <img src="zasoby/branding/ikony/handover.svg" width="24" align="absmiddle"> `/relai-handover` | składa pakiet przekazania projektu w jednym pliku HTML do wysłania dalej |
| <img src="zasoby/branding/ikony/tour.svg" width="24" align="absmiddle"> `/relai-tour` | oprowadza po projekcie wyłącznie na podstawie jego dokumentów; niczego nie zapisuje |
| <img src="zasoby/branding/ikony/help.svg" width="24" align="absmiddle"> `/relai-help` | pokazuje ściągę projektu — komendy, frazy i zachowania automatyczne |
| <img src="zasoby/branding/ikony/adopt.svg" width="24" align="absmiddle"> `/relai-adopt` | przenosi zastany projekt na strukturę RelAI z backupem-bramką i ścieżką cofnięcia |
| <img src="zasoby/branding/ikony/update.svg" width="24" align="absmiddle"> `/relai-update` | dociąga projekt do wersji pluginu; pokazuje różnice, zmienia wyłącznie za zgodą |
| <img src="zasoby/branding/ikony/branch.svg" width="24" align="absmiddle"> `/relai-branch` | odkłada boczny wątek jako odnogę: karta i gotowy prompt świeżej sesji, plan bez zmian |
| <img src="zasoby/branding/ikony/clean.svg" width="24" align="absmiddle"> `/relai-clean` | pokazuje artefakty po zamkniętych etapach w grupach i kasuje wyłącznie te, na które powiesz „tak" |
| <img src="zasoby/branding/ikony/models.svg" width="24" align="absmiddle"> `/relai-models` | odświeża listę modeli narzędzia: zgoda na sieć, źródło albo pytanie do Ciebie, różnica przed zapisem |
| <img src="zasoby/branding/ikony/crew.svg" width="24" align="absmiddle"> `/relai-crew` | zamienia sesję w orkiestratora: fale zadań bez konfliktów plików, delegacja i przegląd krzyżowy |
| <img src="zasoby/branding/ikony/prompt.svg" width="24" align="absmiddle"> `/relai-prompt` | zamienia podyktowane zdanie w precyzyjny prompt: oryginał obok propozycji, każde dopowiedzenie oznaczone, wykonanie czeka na zgodę. Tryb ciągły robi to z każdym promptem — ale najpierw pyta raz na sesję, czy ma działać (`on` / `off` / `off --globalnie` przestawiają go bez edycji pliku) |
...
Jedenaście hooków w Node.js, zero zależności npm. **Dwa blokują, sześć ostrzega, trzy pracują cicho.**
Niżej to, czego pilnują; pełna lista hooków z odpowiednikami w Cursorze i Codeksie stoi w tabeli
[„Co pilnuje tylko Claude Code"](../README.md#co-pilnuje-tylko-claude-code) w README.
...
| **„gotowe" z dowodem** | zadanie, które zmieniło kod, kończy się uruchomieniem testów Twojego projektu i przeglądem własnego diffu; zgłoszenie ma trzy linie: testy z wynikiem, zmienione pliki, ryzyka — brak testów jest powiedziany wprost, nie przemilczany (od 2.7.0) |
| **domknięcia rytuału** | na starcie sesji jedno zdanie, gdy etap jest zamknięty bez wpisu w dzienniku albo bez promptu następnego etapu, gdy artefakt z rejestru zmienił się bez podbicia wersji albo gdy `AGENTS.md`-kopia rozjechała się z `CLAUDE.md`; przy zgodnym stanie cisza (od 2.7.0) |
| **członków załogi** | w `/relai-crew` meldunek bez polecenia z wynikiem nie jest ukończeniem: najwyżej dwie automatyczne kontynuacje, potem sprawa wraca do Ciebie; subagenci dostają tylko duże, niezależne zadania (od 2.7.0) |
...
Przykład, na którym powstał sam RelAI: [PLAN.html](archiwum/plany/BUDOWA_RELAI/PLAN.html)
...
- **Dokumenty rosną.** W długim projekcie dziennik potrafi urosnąć do rozmiarów, które warto
  rotować. Przy „kończymy na dziś" najstarsza historia ponad progiem sama przenosi się do archiwum
  (w całości, z linkiem w żywym pliku); progi i wyłącznik stoją w `docs/USTAWIENIA.md`.
...
│       ├── rules/               #   trzy reguły .mdc: rdzeń i guardraile zawsze, planowanie na żądanie
...
Instrukcja instalacji krok po kroku: [Instalacja → B. Cursor](INSTALACJA.md#b-cursor--adapter-bez-claude-code).
Tutaj rzecz o tym, **co ta instalacja daje** i czym adapter różni się od pluginu.

Warstwą nośną procesu są reguły `.cursor/rules/relai-*.mdc` z `alwaysApply: true` (rdzeń i guardraile;
reguła planowania od 2.7.0 wchodzi na żądanie, po opisie) — wchodzą
...
[docs/PRZENOSNOSC.md](PRZENOSNOSC.md), sekcja 3.
