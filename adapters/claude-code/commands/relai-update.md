...
**Ta komenda pochodzi z RelAI 2.2.0** — ta liczba to wersja docelowa aktualizacji. Komenda jest
...
| Obszar | Stan docelowy 2.2.0 | Jak sprawdzasz |
|---|---|---|
...
| `docs/KOMENDY.md` | nagłówek `RelAI 2.2.0`, tabela **czternastu** komend (w tym `/relai-adopt`, `/relai-update`, `/relai-branch`, `/relai-clean`, `/relai-models`, `/relai-crew`, `/relai-prompt`), aktualna sekcja zachowań automatycznych — z rotacją dokumentów, budżetem startu sesji, przeglądem spraw przeterminowanych, sprzątaniem artefaktów roboczych, wiekiem listy modeli, sygnałem rozjazdu stanu, kontrolą podpisu i bramkami manualnymi — oraz akapit „Do włączenia ręcznie" o skanie sekretów przy commicie, ze zdaniem prawdziwym dla tego projektu (zainstalowany czy nie) | plik regenerowany wg `SPEC_KOMENDY.md`; wiersze oznaczone „lokalne" **przeżywają** regenerację |
...
| marker wersji | `Wersja RelAI: 2.2.0` | zmiana **na końcu**, po wykonaniu zatwierdzonych zmian |
...
2. Propozycja commita: `chore: update RelAI project to 2.2.0`. Commit wyłącznie za zgodą.
