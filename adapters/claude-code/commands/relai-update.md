...
**Ta komenda pochodzi z RelAI 2.3.0** — ta liczba to wersja docelowa aktualizacji. Komenda jest
...
| Obszar | Stan docelowy 2.3.0 | Jak sprawdzasz |
|---|---|---|
...
| `docs/USTAWIENIA.md` | wiersz `Tryb ciągły` z wartością `włączony` albo `wyłączony` na **początku** komórki (nowość 2.3.0; format i wartość domyślna: `SPEC_USTAWIENIA.md`) | wiersz **już obecny — nie nadpisujesz**, także gdy stoi w nim `wyłączony`: to świadomy wybór projektu (R6). Brak wiersza → dopisujesz go z dzisiejszą datą i wartością domyślną **`wyłączony`** — tryb ciągły włącza się jawnie, bo przerabia każdy prompt merytoryczny. Wyłącznik jest **osobny** od pozostałych i od wiersza `Model optymalizatora` |
| marker wersji | `Wersja RelAI: 2.3.0` | zmiana **na końcu**, po wykonaniu zatwierdzonych zmian |
...
2. Propozycja commita: `chore: update RelAI project to 2.3.0`. Commit wyłącznie za zgodą.
