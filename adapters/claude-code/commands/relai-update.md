...
**Ta komenda pochodzi z RelAI 2.7.0** — ta liczba to wersja docelowa aktualizacji. Komenda jest
...
| Obszar | Stan docelowy 2.7.0 | Jak sprawdzasz |
|---|---|---|
...
| `CLAUDE.md` | linia fraz sesji pod listą rytuału startu; od 1.8.0 opis frazy „kończymy na dziś" niesie **sprzątanie artefaktów roboczych** obok syncu dokumentów, wpisu i przeglądu ryzyk | linii brak → do dodania wg `SPEC_CLAUDE_MD.md` (sekcja „Linia fraz sesji"). Linia **jest**, ale bez członu o sprzątaniu → dopisujesz sam człon, nie przepisujesz linii; nie ruszasz przy tym niczego innego w pliku |
...
| `docs/KOMENDY.md` | nagłówek `RelAI 2.7.0`, tabela **czternastu** komend (w tym `/relai-adopt`, `/relai-update`, `/relai-branch`, `/relai-clean`, `/relai-models`, `/relai-crew`, `/relai-prompt`), aktualna sekcja zachowań automatycznych — z rotacją dokumentów, budżetem startu sesji, przeglądem spraw przeterminowanych, sprzątaniem artefaktów roboczych, wiekiem listy modeli, sygnałem rozjazdu stanu, siatką rytuału „Na koniec" i wersji artefaktów, krokiem „gotowe z wynikiem testów", kontrolą podpisu i bramkami manualnymi — oraz akapit „Do włączenia ręcznie" o skanie sekretów przy commicie, ze zdaniem prawdziwym dla tego projektu (zainstalowany czy nie) | plik regenerowany wg `SPEC_KOMENDY.md`; wiersze oznaczone „lokalne" **przeżywają** regenerację |
...
| `docs/USTAWIENIA.md` | wiersz `Lista modeli` z wartością `włączona` na **początku** komórki i progiem w dniach (nowość 1.9.0; format i wartość domyślna: `SPEC_USTAWIENIA.md`) | wiersz **już obecny — nie nadpisujesz**, także gdy stoi w nim `wyłączona` albo własny próg (R6). Brak wiersza → dopisujesz go z dzisiejszą datą i wartością domyślną `włączona · 7 dni`. Wyłącznik jest **osobny** od pozostałych; samej listy `.claude/relai/MODELE-<narzędzie>.md` **nie ruszasz** — kopia w projekcie jest trwała i przeżywa aktualizację |
| `docs/USTAWIENIA.md` | wiersz `Tryb ciągły` z wartością `włączony` albo `wyłączony` na **początku** komórki (nowość 2.2.0; format i wartość domyślna: `SPEC_USTAWIENIA.md`) | wiersz **już obecny — nie nadpisujesz**, także gdy stoi w nim `wyłączony`: to świadomy wybór projektu (R6). Brak wiersza → dopisujesz go z dzisiejszą datą i wartością domyślną **`wyłączony`** — tryb ciągły włącza się jawnie, bo przerabia każdy prompt merytoryczny. Wyłącznik jest **osobny** od pozostałych i od wiersza `Model optymalizatora` |
| marker wersji | `Wersja RelAI: 2.7.0` | zmiana **na końcu**, po wykonaniu zatwierdzonych zmian |
...
Jeden wyjątek dotyczy projektu, w którym hook **już jest** i pochodzi sprzed 1.9.3: układ plików
w `.git/hooks/` zmienił się (shim powłokowy plus `relai-pre-commit.cjs` i `relai-secret-scan.cjs`
zamiast bezrozszerzeniowej kopii i pliku `.js`), a stary układ **przewraca się w projekcie
z `"type": "module"`**. Rozpoznajesz to po obecności `.git/hooks/relai-secret-scan.js`. Hook
z **1.9.2** ma juz nowy uklad, ale niesie dwie regresje zamkniete w 1.9.3 — blokade poprawnego
odczytu sekretu ze srodowiska i deinstalacje psujaca cudzy hook — wiec tez wymaga ponownej
instalacji. Wtedy
w podsumowaniu stoi zdanie, że ponowna instalacja jest **wymagana**, a nie możliwa — razem
z poleceniem. Instalacji nadal nie wykonujesz za człowieka.
...
2. Propozycja commita: `chore: update RelAI project to 2.7.0`. Commit wyłącznie za zgodą.
